...
if (genresCookie.value) {
	console.log((genresCookie.value))
	await fetchRecommendedBooks((genresCookie.value) as unknown as string[]);
}
...
onMounted(async () => {
	...
	if (!genres.length) {
		...
		if (storedGenres) {
			...
			genresCookie.value = JSON.stringify(genres);
		}
